...
import { updateTaskTime, updateTaskQuestions, createBulkExpenditures, ExpenditureItem } from "../../_lib/webexpApi";
...
export default function Task3QuestionsPage() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Submit timing data
      await updateTaskTime(taskId, participantId, 3, timeSeconds);
      
      // Submit questions
      await updateTaskQuestions(taskId, participantId, 3, question1, question2);
      ...
      if (entries.length > 0) {
        const expenditures: ExpenditureItem[] = entries.map((e) => ({
          name: e.name,
          date_of_expense: e.date,
          amount: parseFloat(e.amount.replace("$", "")),
          category: e.category || undefined,
          notes: e.note || undefined,
          status: "Pending",
        }));
        await createBulkExpenditures(taskId, participantId, 3, expenditures);
      }
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col bg-white px-4 py-6 text-sm text-zinc-900">
      <header className="mb-6 text-center">
        ...
        <p className="mt-1 text-xs text-zinc-500">Task 3: Manual Entry - Questions</p>
      </header>
